...
export default function PortfolioGallery({ images }) {
    ...
    return (
        <FadeInOnView>
            ...
            >
                {images.map((item, index) => (
                    <a
                        key={index}
                        data-src={item.img}
                        className="group relative block aspect-square rounded-xl overflow-hidden cursor-zoom-in shadow-sm hover:shadow-md transition-all border border-slate-100"
                    >
                        <Image
                            src={item.img}
            ...
            <p className="text-center text-slate-400 text-xs mt-6 italic font-subject">
